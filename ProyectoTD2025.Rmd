...
(Esta es sencilla buscais en el df de info_ticket cual tiene mayor importe y luego mostrais todos los productos asociados a la factura simplificada de ese ticket, en este caso para graficarlo podeis simplemente meter todos los tickets en una grafica y señalar con un simbolo en el plot el ticket que tenga el valor mas alto )

- ¿Cuál es el importe medio de los tickets por municipio? ¿Se observan diferencias significativas entre zonas?
(agrupais los datos por ciudad y mostrais en graficas diferentes (por ciudad) la media del importe de esos tickets, y luego respondeis a la pregunta)

- ¿Cuál es la procedencia de los tickets? ¿Qué ciudad/pueblo tiene un mayor número de tickets?
(Decis cuales son los pueblos que tenemos en la tabla y luego agrupais los datos por ciudad y sumais el numero de tickets, luego lo podeis mostrar en un grafico de barras)
...
frutas_verduras <- productos %>%
  filter(str_detect(tolower(Producto), "banana|plátano|tomate|cebolla|zanahoria|lechuga|manzana|pera|patata|limón|naranja|pepino|uvas|kiwi")) %>%
  filter(!is.na(PrecioKg))

# Seleccionar productos de pescado
pescado <- productos %>%
  filter(str_detect(tolower(Producto), "pescado|merluza|salmón|lubina|atún|dorada|boquerón|bacalao")) %>%
  filter(!is.na(PrecioKg))
