...
```{r}
fichero <- "./data/20240801 REFERENCIA Mercadona 81,34 Ôé¼.pdf"
  data <- pdf_text(fichero)
  lineas <- str_split(data,"\n")[[1]]
  lineas
```
...
funcion_df_parte1 <- function(lineas){  
  ...
  C.Postal <- if (length(telefono_linea) > 0) substr(C.Postal_linea, start = 1, stop = 5) else NA
  
  # Municipio
  Municipio_linea <- str_trim(lineas[3])
  Municipio <- if (length(telefono_linea) > 0) substring(Municipio_linea, first = 7) else NA
...
df_info_ticket_parte1 <- funcion_df_parte1(lineas)
```
...
parse_lineas_ticket <- function(lineas, id_ticket) {
  ...
  for (linea in lineas) {
    ...
    if (str_detect(linea, patron_unidad_varias)) {
    ...
    else if (str_detect(linea_sin_trim, "^\\s{2}\\S")) {
      nombre_categoria <- str_trim(linea_sin_trim)
      nombre_producto <- ""
    }
      ...
      nombre_producto <- str_trim(linea_sin_trim)
    }
...
productos <- parse_lineas_ticket(lineas_productos, substr(lineas[6], start = 30, stop = 45))
...
funcion_df_parte3 <- function(lineas){
  ...
  tipo_tarjeta <- if (length(tipo_tarjeta) > 0) {
    str_extract(tipo_tarjeta, "(?<=\\d\\s)\\w+")
  } else {
    NA
  }
...
fichero <- "data/20231125 Mercadona 37,76 Ôé¼.pdf"
  data <- pdf_text(fichero)
  lineas <- str_split(data,"\n")[[1]] 
  
prueba <- funcion_df_parte3(lineas)
```
...
for (ticket_pdf in tickets_mercadona) {
  ...
  productosUWU <- parse_lineas_ticket(lineas_productos, substr(lineas[6], start = 30, stop = 45))
  parte3 <- funcion_df_parte3(lineas)
  
  DF_INFO <- rbind(df_info_ticket_parte1,parte1)
  DF_Productos <- rbind(productos,productosUWU)
  DF_INFO2 <- rbind(df_info_ticket,parte3)
}
```
