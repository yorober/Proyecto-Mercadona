...
parse_lineas_ticket <- function(lineas_productos, id_ticket) {
  ...
  for (linea in lineas) {
    if (any(str_detect(lineas_productos, "PARKING"))) next
...
productos <- parse_lineas_ticket(lineas, substr(lineas[6], start = 30, stop = 45))
...
=======
  nc <- str_extract(lineas_final[grepl("^\\s*N\\.C\\s*:", lineas_final)], "(?<=N.C:\\s)\\d+")
>>>>>>> 6ae2c6eae54804e91de5d4527e5e4c3ef3c51fbd
...
=======
    str_extract(tipo_tarjeta, "(DEBIT|CREDIT|CONTADO|VISA|MASTERCARD)\\s+[A-Z]+")
>>>>>>> 6ae2c6eae54804e91de5d4527e5e4c3ef3c51fbd
  } else {
    NA
  }
...
for (ticket_pdf in tickets_mercadona) {
  ...
  productosUWU <- parse_lineas_ticket(lineas, substr(lineas[6], start = 30, stop = 45))
  parte3 <- funcion_df_parte3(lineas)
  
  df_info_ticket_parte1 <- rbind(df_info_ticket_parte1,parte1)
  productos <- rbind(productos,productosUWU)
