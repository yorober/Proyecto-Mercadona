...
>>>>>>> 91b6b8307bfddcbe5e9889165bf1339fd4fce64f
  lineas <- tryCatch({
...
funcion_df_parte3 <- function(lineas){
  ...
  parkingE <- hms(paste0(parkingE, ":00"))
  
  
  #Parking Salida
  PSlineas <- lineas[grepl("SALIDA \\d{2}:\\d{2}", lineas)]
  PSlineas <- str_extract(PSlineas, "SALIDA \\d{2}:\\d{2}")
  PSlineas <- ifelse(is.na(PSlineas), "00:00", sub("SALIDA", "", PSlineas))
  parkingS <- str_trim(PSlineas)
  parkingS <- hms(paste0(parkingS, ":00"))
